      ...
      <div className="container" style={{ padding: '0 clamp(1rem, 3vw, 2rem)' }}>
        <div className="contact-layout" style={{
          ...
          <div className="contact-details">
            ...
            <h2 className="section-title" style={{ marginBottom: '1.2rem', fontSize: 'clamp(1.8rem, 4vw, 2.8rem)' }}>Resort Contact Details</h2>
            <div className="divider" />

            <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(1.2rem, 2vw, 1.8rem)', marginTop: 'clamp(1.5rem, 2.5vw, 2rem)' }}>
              {contactItems.map((item) => (
                <div key={item.title} className="contact-item" style={{ display: 'flex', gap: 'clamp(0.8rem, 1.5vw, 1.2rem)', alignItems: 'flex-start' }}>
                  <div style={{
                  ...
                  <div>
                    <h3 style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 'clamp(0.85rem, 1.2vw, 0.95rem)', color: 'var(--color-text)', marginBottom: '4px' }}>{item.title}</h3>
                    <p style={{ fontSize: 'clamp(0.8rem, 1.1vw, 0.88rem)', color: 'var(--color-text-mid)', lineHeight: 1.6 }}>{item.text}</p>
                    {item.email && <p style={{ fontSize: 'clamp(0.8rem, 1.1vw, 0.88rem)', color: 'var(--brand-cyan-muted)', marginTop: '2px', fontWeight: 500 }}>{item.email}</p>}
                    {item.link && (
                      <a
                        href={item.link}
                        target="_blank"
            ...
            {/* Map */}
            <div className="contact-map" style={{ marginTop: 'clamp(2rem, 3vw, 2.5rem)', borderRadius: 'var(--radius)', overflow: 'hidden', height: 'clamp(200px, 30vw, 240px)', border: '1px solid rgba(6,181,211,0.12)' }}>
              <iframe
          ...
          <div className="contact-form" style={{
            ...
            {submitted ? (
              <div style={{
                ...
                <h4 style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 'clamp(1.1rem, 1.6vw, 1.3rem)', color: 'var(--color-text)', marginBottom: '0.5rem' }}>Message Sent!</h4>
                <p style={{ fontSize: 'clamp(0.82rem, 1vw, 0.9rem)', color: 'var(--color-text-soft)', lineHeight: 1.7 }}>
                  Our guest relations desk will reach out to you within 24 hours.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(0.8rem, 1.3vw, 1.2rem)' }}>
                {[
